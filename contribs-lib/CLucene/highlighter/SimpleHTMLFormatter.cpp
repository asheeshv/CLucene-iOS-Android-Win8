...
TCHAR* SimpleHTMLFormatter::highlightTerm(const TCHAR* originalText, const TokenGroup* tokenGroup)
{
	if(tokenGroup->getTotalScore()>0){
		std::string origText = lucene_wcstoutf8string(originalText, wcslen(originalText));
//		printf("SimpleHTMLFormatter::highlightTerm, originalText is:%s\n\n", origText.c_str());
//		printf("start offset is %d, and end offset is:%d\n\n",tokenGroup->getStartOffset(), tokenGroup->getEndOffset());
		StringBuffer sb;
		sb.append(_preTag);
