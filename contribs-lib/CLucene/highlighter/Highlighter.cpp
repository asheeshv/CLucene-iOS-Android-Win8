				...
				if((tokenGroup->getNumTokens()>0)&&(tokenGroup->isDistinct(&token))){
					...
					// markup the cached token group info
					 ...
					 endOffset = tokenGroup->getEndOffset();
						//printf("Highlighter::getBestTextFragments, startOffset is:%d and endOffset is%d\n",startOffset, endOffset);
					 ...
					 TCHAR* encoded = _encoder->encodeText(substringBuffer);
						//printf("Highlighter::encoded text is:%s\n\n",lucene_wcstoutf8string(encoded, wcslen(encoded)).c_str());
