...
void IndexWriter::init(Directory* d, Analyzer* a, const bool create, const bool closeDir,
     ...
     #ifdef __ANDROID__
	__android_log_print(ANDROID_LOG_INFO,"IndexWriter::Init", "beginning of the function");
    #endif
  ...
  this->writeLock = NULL;
 #ifdef __ANDROID__
  __android_log_print(ANDROID_LOG_INFO,"IndexWriter::Init", "this is before trying to acquire a lock");
#endif
  if (create) {
  ...
  try {
    ...
    #ifdef __ANDROID__
  __android_log_print(ANDROID_LOG_INFO,"IndexWriter::Init", "lock acquired now create a file");
#endif
  try {
    if (create) {
 ...
 #ifdef __ANDROID__
    	  __android_log_print(ANDROID_LOG_INFO,"IndexWriter::Init", "LuceneErrorOccurred and error is:%s, and number is:%d",e.what(),e.number());
#endif
        if ( e.number() != CL_ERR_IO )
        	{
                 #ifdef __ANDROID__
        	__android_log_print(ANDROID_LOG_INFO,"IndexWriter::Init", "LuceneError and is not CL_ERR_IO, error is:%s",e.what());
#endif
        		throw e;
        	}
 ...
 #ifdef __ANDROID__
      __android_log_print(ANDROID_LOG_INFO,"IndexWriter::Init", "segmentInfos->write(directory)");
#endif
    } else {
      segmentInfos->read(directory);
    }
#ifdef __ANDROID__
    __android_log_print(ANDROID_LOG_INFO,"IndexWriter::Init", "we should reach here");
#endif
